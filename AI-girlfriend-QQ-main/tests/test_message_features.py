...
def test_split_reply_text_never_truncates_the_final_thought() -> None:
    text = "第一点已经确认。第二点也已经确认。第三点需要保留完整结论。最后一句不能被省略。"
    parts = split_reply_text(text, max_chars=10, max_parts=2)
...
def test_sticker_semantic_filename_selects_best_flat_image(tmp_path) -> None:
    sticker_dir = tmp_path / "聊天表情"
    sticker_dir.mkdir()
    matching = sticker_dir / "让我想想.png"
    unrelated = sticker_dir / "哈哈哈.png"
    matching.write_bytes(b"fake")
    ...
    assert choice is not None
    assert choice.file_url == str(matching.resolve())


def test_sticker_semantic_filename_supports_gif(tmp_path) -> None:
    sticker_dir = tmp_path / "聊天表情"
    sticker_dir.mkdir()
    animated = sticker_dir / "生气.gif"
    excessive = sticker_dir / "攻击，火箭拳.jpg"
    animated.write_bytes(b"GIF89a")
    excessive.write_bytes(b"fake")
    manager = StickerManager(sticker_dir)

    choice = manager.choose("这也太气人了", "我真的生气了。", chance=1.0, cooldown_seconds=0)

    assert choice is not None
    assert choice.file_url == str(animated.resolve())


def test_external_sticker_library_falls_back_to_trigger_directory(tmp_path) -> None:
    sticker_dir = tmp_path / "external"
    trigger_root = tmp_path / "managed"
    sticker_dir.mkdir()
    old_image = trigger_root / "proud" / "atri.png"
    old_image.parent.mkdir(parents=True)
    old_image.write_bytes(b"fake")
    trigger_file = trigger_root / "triggers.json"
    trigger_file.write_text(
        json.dumps(
            {"trigger_words": {"高性能": {"emotion": "proud", "file": "proud/atri.png"}}},
            ensure_ascii=False,
        ),
        encoding="utf-8",
    )
    manager = StickerManager(sticker_dir, trigger_file)

    choice = manager.choose("高性能", "任务完成", chance=0.0)

    assert choice is not None
    assert choice.file_url == str(old_image.resolve())
...
def test_outgoing_record_message_uses_onebot_segment() -> None:
    message = outgoing_to_onebot_message(OutgoingMessage("record", "D:\\voice\\atri.wav"))

    assert message == [
        {"type": "record", "data": {"file": "D:\\voice\\atri.wav", "cache": 0}}
    ]
...
def test_group_public_memory_merges_into_same_private_person_without_leaking_private_to_group(
    tmp_path,
) -> None:
    store = UserMemoryStore(tmp_path / "users.json")

    store.observe_user("private:10001", "我的生日是8月12日", now=1000)
    store.observe_user("private:10001", "对了，我生日是8月12日，别忘", now=1010)
    store.observe_group_message(
        20001,
        10001,
        "我喜欢喝咖啡",
    ...
    private_memory = store.profile("private:10001", now=1040)["structured_memory"]
    group_context = store.recall_context(
    ...
    assert any(
        item["category"] in {"interest", "preference"}
        and "咖啡" in str(item.get("value") or "")
        for item in private_memory["l1"]
    )
    assert "咖啡" in group_context
    assert "8月12日" not in group_context


def test_negative_preference_correction_removes_positive_interest(tmp_path) -> None:
    store = UserMemoryStore(tmp_path / "users.json")

    store.observe_user("private:10001", "我喜欢喝奶茶", now=1000)
    store.observe_user("private:10001", "我真的喜欢喝奶茶", now=1010)
    assert "奶茶" in store.recall_context("private:10001", "今天喝点什么", now=1020)

    store.observe_user("private:10001", "不喝奶茶，别再记成我爱喝奶茶了", now=1030)
    memory = store.profile("private:10001", now=1040)["structured_memory"]
    serialized_l1 = json.dumps(memory["l1"], ensure_ascii=False)
    serialized_candidates = json.dumps(memory["candidates"], ensure_ascii=False)

    assert "喜欢的食物:奶茶" not in serialized_l1
    assert "兴趣:奶茶" not in serialized_l1
    assert "讨厌:奶茶" in serialized_candidates or "不喜欢" in serialized_candidates


def test_memory_store_reloads_webui_edits_before_next_save(tmp_path) -> None:
    memory_path = tmp_path / "users.json"
    ...
    admin = MemoryAdmin(memory_path, tmp_path / "backups")
    ...
    assert "兴趣:奶茶" not in serialized
...
def test_group_proactive_stops_after_silence_days(tmp_path) -> None:
    store = UserMemoryStore(tmp_path / "users.json")
    conversation_id = "group:20001"
    day = 24 * 60 * 60
    store.remember_target(conversation_id, {"message_type": "group", "group_id": 20001})
    store.observe_user(conversation_id, "hello group", now=1000)

    assert store.due_group_targets(0, 0, 3, max_silence_days=3, now=1000 + 2 * day)
    assert store.due_group_targets(0, 0, 3, max_silence_days=3, now=1000 + 4 * day) == []

    store.observe_user(conversation_id, "back again", now=1000 + 4 * day + 10)

    assert store.due_group_targets(0, 0, 3, max_silence_days=3, now=1000 + 4 * day + 20)
...
def test_iteration_accepts_quality_correction_and_rejects_bad_correction(tmp_path) -> None:
    ...
    assert "回复失败" in accepted
    assert "回复失败" in rejected
    assert "未使用本地内容模板" in accepted
    assert "未使用本地内容模板" in rejected
...
def test_private_affection_extremes_rebalance_toward_normal_range(tmp_path) -> None:
    store = UserMemoryStore(tmp_path / "users.json")
    store.remember_target("private:1", {"message_type": "private", "user_id": 1})
    store.observe_user("private:1", "你好", now=1000)
    store.set_affection("private:1", 95)

    store.due_idle_targets(idle_minutes=30, cooldown_minutes=120, now=1000 + 6 * 24 * 60 * 60)

    assert 55 < store.profile("private:1", now=1000 + 6 * 24 * 60 * 60)["affection_score"] < 95

    store.set_affection("private:1", 10)
    store.due_idle_targets(idle_minutes=30, cooldown_minutes=120, now=1000 + 12 * 24 * 60 * 60)
    assert 10 < store.profile("private:1", now=1000 + 12 * 24 * 60 * 60)["affection_score"] < 55


def test_explicit_personal_event_is_reminded_privately_once(tmp_path) -> None:
    store = UserMemoryStore(tmp_path / "users.json")
    owner = 10001
    now = datetime(2026, 8, 3, 12, 0, tzinfo=timezone(timedelta(hours=8)))
    due_at = datetime(2026, 8, 4, 9, 1, tzinfo=timezone(timedelta(hours=8)))
    store.remember_target("private:10001", {"message_type": "private", "user_id": owner})

    store.observe_user("private:10001", "明天是我的生日", now=now.timestamp(), is_owner=True)
    due = store.due_personal_events((owner,), now=due_at.timestamp())

    assert len(due) == 1
    assert due[0][1] == {"message_type": "private", "user_id": owner}
    assert due[0][2]["kind"] == "birthday"
    store.mark_personal_event_sent("private:10001", due[0][2]["id"], now=due_at.timestamp())
    assert store.due_personal_events((owner,), now=due_at.timestamp() + 60) == []


def test_affection_daily_caps_prevent_message_spam_from_inflating_score(tmp_path) -> None:
    store = UserMemoryStore(tmp_path / "users.json")

    for index in range(30):
        store.observe_user("private:1", "谢谢你，高性能", now=1000 + index)

    score = store.profile("private:1", now=1100)["affection_score"]
    assert 50 < score <= 51.4


def test_lore_trigger_with_disabled_model_returns_diagnostic(tmp_path) -> None:
    config = BotConfig(
        bot_qq=100000001,
        host="127.0.0.1",
        port=8765,
